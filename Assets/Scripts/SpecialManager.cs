...
public class SpecialManager : MonoBehaviour
{
    ...
    public List<Special> allEnemySpecials;
    ...
    void Awake()
    {
        ...
        allEnemySpecials = new List<Special>();
        //Ally specials
        ...
        allSpecials.Add(new Special("Mana Drain", "Decrease an enemy's current mp by half.", 9, 0, isModifier: true, modifierStats: new List<float[]>(){new float[]{-6, 0.5f, 1}}));
        allSpecials.Add(new Special("Phalanx", "Increase all allies physical defense by 0.5x for 2 turns.", 10, 3, isModifier: true, modifierStats: new List<float[]>(){new float[]{3, 0.5f, 1}}));
        allSpecials.Add(new Special("Bullseye", "Strike an enemy powerfully twice for 1x physical.", 9, 0, intensity: 1f, numberOfHits: 2, isPhysical: true));

        //Enemy specials
        allEnemySpecials.Add(new Special("Lightning Bolt", "Hits an enemy powerfully for 1.25x mental.", 7, 0, intensity: 1.25f, numberOfHits: 1, isMental: true));
        allEnemySpecials.Add(new Special("Sweeping Hail", "Hits all enemies for 0.75x mental", 12, 1, intensity: 1f, numberOfHits: 1, isMental: true));
        allEnemySpecials.Add(new Special("Channel the Gods", "Buffs self for 0.25x mental for 3 turns.", 5, 4, intensity: 1f, numberOfHits: 1, isModifier: true, modifierStats: new List<float[]>(){new float[]{2, 0.25f, 3}}));
        allEnemySpecials.Add(new Special("Wind of Decay", "Debuffs all allies mental defense by 0.33x for 3 turns.", 10, 1, isModifier: true, modifierStats: new List<float[]>(){new float[]{-4, 0.33f, 3}}));

        allEnemySpecials.Add(new Special("Smite", "Hits an enemy powerfully for 1.75x mental.", 9, 0, intensity: 1.75f, numberOfHits: 1, isMental: true));
        allEnemySpecials.Add(new Special("Fists of Judgement", "Hits all enemies twice for 0.4x physical.", 12, 1, intensity: 0.4f, numberOfHits: 2, isPhysical: true));
        allEnemySpecials.Add(new Special("Eldritch Regeneration", "Heals self for 3x mental 3 times", 7, 4, intensity: 5f, numberOfHits: 3, isMental: true, isHealing: true));
        allEnemySpecials.Add(new Special("Mana Vacuum", "Halves all enemies' current mp.", 15, 1, isModifier: true, modifierStats: new List<float[]>(){new float[]{-6, 0.5f, 1}}));
        allEnemySpecials.Add(new Special("Health Vacuum", "Halves all enemies' current hp.", 15, 1, isModifier: true, modifierStats: new List<float[]>(){new float[]{-5, 0.5f, 1}}));
        allEnemySpecials.Add(new Special("Wave of Death", "Hits all enemies 3 times for 0.33x mental.", 15, 1, intensity: 0.33f, numberOfHits: 3, isMental: true));
    ...
    public void activateSpecial(int specialIndex, int playerIndex = 0, int enemyIndex = 0, bool isEnemyAction = false)
    {
        if(!isEnemyAction)
        {
            activated = true;
    ...
    public IEnumerator useSpecial()
    {
        while(true)
        {
            yield return new WaitUntil(() => activated || enemyActivated);
            ...
            else if(enemyActivated)
            {
                Character thePlayer = theBattleManager.enemyList[enemyIndex].GetComponent<Character>();
                if(theSpecial.targeting == 0)
                {
                ...
                else if(theSpecial.targeting == 2)
                {
                    ...
                    for(int i = 0; i < theSpecial.numberOfHits; i++)
                    {
                        if(theEnemy != null)
                        {
                            if(theSpecial.isHealing)
                            {
                                battleHud.spawnParticle(2, theBattleManager.enemyList[playerIndex]);
                            }
                            else if(theSpecial.isPhysical)
                            {
                                battleHud.spawnParticle(0, theBattleManager.enemyList[playerIndex]);
                            }
                            else if(theSpecial.isMental)
                            {
                                battleHud.spawnParticle(1, theBattleManager.enemyList[playerIndex]);
                            }
                            theEnemy.stats.currenthp -= damage;
                            theEnemy.stats.currenthp += healing;
                            Debug.Log("HEALED ALLY FOR " + healing + " DAMAGED ALLY FOR " + damage);
                    ...
                    if(theSpecial.isModifier && theEnemy != null)
                    {
                        for(int i = 0; i < theSpecial.modifierStats.Count; i++)
                        {
                            ...
                            if(theSpecial.modifierStats[i][0] < 0)
                            {
                                battleHud.spawnParticle(3, theBattleManager.enemyList[playerIndex]);
                            }
                            else if(theSpecial.modifierStats[i][0] > 0)
                            {
                                battleHud.spawnParticle(4, theBattleManager.enemyList[playerIndex]);
                            }
                            yield return new WaitForSeconds(0.25f);
                        }
                    }
